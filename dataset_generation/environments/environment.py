...
from dataset_generation.utils.global_vars import PIXEL_SIZE, CAMERA_CONFIG, BOUNDS, IN_SHAPE
from dataset_generation.utils import pybullet_utils
...
class Environment(gym.Env):
    ...
    def __init__(self,
        ...
        """
        self.scene_graph_generator = SceneGraphWithGDino(disable_load=True)
    ...
    def reset(self):
        ...
        obs, cliport_obs, _, _, _, _ = self.step()

        return obs, cliport_obs
    ...
    def step(self, action=None, oracle=False):
        ...
        if action is not None:
            ...
            if timeout:
                obs = {'color': (), 'depth': ()}
                cliport_obs = {'color': (), 'depth': ()}
                for idx, config in enumerate(self.agent_cams):
                    color, depth, _ = self.render_camera(config)
                    obs['color'] += (color,)
                    obs['depth'] += (depth,)
                    if idx > 0:
                        cliport_obs['color'] += (color,)
                        cliport_obs['depth'] += (depth,)
                _, hmap = utils.get_fused_heightmap(
                    obs, self.agent_cams, self.task.bounds,
                    pix_size=self.pix_size
                )
                obs['height'] = hmap
                return obs, cliport_obs, 0.0, True, self.info, self.cliport_info

        # Step simulator asynchronously until objects settle.
        for i in range(1000):
            if not self.is_static: # to pass unstable situation faster
        ...
        if oracle:
            ...
            cliport_info.update(self.cliport_info)
        else:
            reward = 0.0
        ...
        obs, cliport_obs = self._get_obs() # get two observation, can be easily used for SREM and CLIPort each

        return obs, cliport_obs, reward, done, self.info, self.cliport_info
    ...
    def update_info_online(self, info):
        ...
        for k, _ in info.items():
            if type(info[k]) != dict or k == 'names':
                continue
            # change info to bbox
            for key, value in info[k].items():
                ...
                boxes[key] = list(chain(*[left, right]))
        
        # generate graph
        graph = self.scene_graph_generator.make_sg(boxes)
        info['graph'] = graph
        return info
    ...
    def info(self):
        ...
        if self.task.goals == []:
            self.obj_ids['move'] = []
        elif self.task.goals is not None: 
        ...
        if len(self.task.goals) > 0:
            info_dict = self.task.goals[0][2][-1][-1] # goal info is used in the task oracle, evaluation
            info['rel_ids'] = []
            info['ref_ids'] = []
            info['relations'] = []
            if 'rel_idxs' in info_dict:
                for rel, ref, relation in zip(info_dict['rel_idxs'], info_dict['ref_idxs'], info_dict['relations']):
                    rel_id = info_dict['obj_ids'][rel]
                    ref_id = info_dict['obj_ids'][ref]
                    info['rel_ids'].append(rel_id)
                    info['ref_ids'].append(ref_id)
                    info['relations'].append(relation)
        return self.update_info_online(info)
    
    @property
    def cliport_info(self):
        """Environment info with object poses, dimensions, and colors."""
    ...
    def start_rec(self, video_filename):
        ...
        self.video_writer = imageio.get_writer(
            os.path.join(
                ...
                f"{self.task.name}_{video_filename}.mp4"
            ),
    ...
    def _get_obs(self):
        ...
        obs = {'color': (), 'depth': ()}
        clip_obs = {'color': (), 'depth': ()}
        for idx, config in enumerate(self.agent_cams):
            color, depth, _ = self.render_camera(config)
            obs['color'] += (color,)
            obs['depth'] += (depth,)
            if idx > 0:
                clip_obs['color'] += (color,)
                clip_obs['depth'] += (depth,)
        _, hmap = utils.get_fused_heightmap(
