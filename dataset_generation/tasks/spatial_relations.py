"""
We add far and close relationships based on task defined in 
https://github.com/ayushjain1144/ebmplanner
...
import random
import numpy as np
from dataset_generation.utils.global_vars import PIXEL_SIZE
...
class CloseSeenColors(SpatialRelations):
    def __init__(self):
        super().__init__()
        self.relation = "close"
    ...
    def get_relation(self, x1, y1, x2, y2, constraint_image):
        w, h = constraint_image.shape
        ctx_x = (x1+x2)//2
        ctx_y = (y1+y2)//2
        for i in range(w):
            for j in range(h):
                dist = math.sqrt((ctx_x-i)**2 + (ctx_y-j)**2)
                if dist < 150:
                    constraint_image[i, j] = 1
    ...
    def reset(self, env):
        ...
        if env.failed_datagen:
            return None
        self.lang_goals[-1] = self.lang_goals[-1].replace("to the close of", "close to")
        if self.save:
            return self.labels

class CloseUnseenColors(CloseSeenColors):
    def __init__(self):
        super().__init__()
...
class FarSeenColors(SpatialRelations):
    def __init__(self):
        super().__init__()
        self.relation = "far"
    ...
    def reset(self, env):
        ...
        if env.failed_datagen:
            return None
        self.lang_goals[-1] = self.lang_goals[-1].replace("to the far of", "far away from")
        if self.save:
            return self.labels

class FarUnseenColors(FarSeenColors):
    def __init__(self):
        super().__init__()
...
class LeftAboveSeenColors(SpatialRelations):
    def __init__(self):
        super().__init__()
        self.relation = "left above"

    def get_colors(self):
        return utils.TRAIN_COLORS

    def get_relation(self, x1, y1, x2, y2, constraint_image):
        if x1 - self.buffer < 0 and y1 - self.buffer < 0:
            return constraint_image
        constraint_image[:max(x1-self.buffer, 0), :max(y1-self.buffer, 0)] = 1
        return constraint_image

class RightAboveSeenColors(SpatialRelations):
    def __init__(self):
        super().__init__()
        self.relation = "right above"

    def get_colors(self):
        return utils.TRAIN_COLORS

    def get_relation(self, x1, y1, x2, y2, constraint_image):
        if x2 + self.buffer >= constraint_image.shape[0] and y1 - self.buffer < 0:
            return constraint_image

        constraint_image[min(x2+self.buffer, constraint_image.shape[0]):, :max(y1-self.buffer, 0)] = 1
        return constraint_image

class LeftBelowSeenColors(SpatialRelations):
    def __init__(self):
        super().__init__()
        self.relation = "left below"

    def get_colors(self):
        return utils.TRAIN_COLORS

    def get_relation(self, x1, y1, x2, y2, constraint_image):
        if x1 - self.buffer < 0 and y2 + self.buffer >= constraint_image.shape[1]:
            return constraint_image
        constraint_image[:max(x1-self.buffer, 0), min(y2+self.buffer, constraint_image.shape[1]):] = 1
        return constraint_image

class RightBelowSeenColors(SpatialRelations):
    def __init__(self):
        super().__init__()
        self.relation = "right below"

    def get_colors(self):
        return utils.TRAIN_COLORS

    def get_relation(self, x1, y1, x2, y2, constraint_image):
        if x2 + self.buffer >= constraint_image.shape[0] and y2 + self.buffer >= constraint_image.shape[1]:
            return constraint_image
        constraint_image[min(x2+self.buffer, constraint_image.shape[0]):, min(y2+self.buffer, constraint_image.shape[1]):] = 1
        return constraint_image
